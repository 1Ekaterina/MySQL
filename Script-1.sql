CREATE TABLE `like_post` (
  `post_id` int(10) unsigned NOT NULL COMMENT 'ссылка на медиа',
...
UPDATE profiles SET status = FLOOR(1 + RAND() * 2);
...
UPDATE media SET media_type_id = FLOOR(1 + RAND() * 100);
...
INSERT INTO friendship_statuses (name) VALUES
 ...
 UPDATE like_media SET updated_at = NOW() WHERE updated_at < created_at; 
 
 UPDATE like_post SET updated_at = NOW() WHERE updated_at < created_at; 
 
 UPDATE like_users SET updated_at = NOW() WHERE updated_at < created_at; 